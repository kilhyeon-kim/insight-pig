...
--
-- 데이터 구조:
--   - TS_INS_WEEK_SUB (GUBUN='DOPE', SORT_NO=1): 지난주 유형별 통계 (1행)
--   - TS_INS_WEEK_SUB (GUBUN='DOPE', SORT_NO=2): 최근1개월 + 당해년도 누계 (1행)
--   - TS_INS_WEEK_SUB (GUBUN='DOPE_R_1', 'DOPE_R_2', ...): 원인별 15개씩 피벗 (병합)
--       - STR_1~STR_15: 원인코드(OUT_REASON_CD)
--       - CNT_1~CNT_15: 지난주 두수
--       - VAL_1~VAL_15: 최근1개월 두수
--       - 프론트에서 TC_CODE_JOHAP(PCODE='031')으로 코드명 조회
--   - TS_INS_WEEK_SUB (GUBUN='DOPE_CHART'): 상태별 차트 (1행)
--
-- OUT_GUBUN_CD 4개 유형 (PCODE='08'):
--   CNT_1: 도태(080001)  CNT_2: 폐사(080002)  CNT_3: 전출(080003)  CNT_4: 판매(080004)
--
-- STATUS_CODE 6개 상태 (PCODE='01'):
--   CNT_1: 후보돈(010001)  CNT_2: 이유모돈(010002)  CNT_3: 임신돈(010003)
--   CNT_4: 포유돈(010004)  CNT_5: 사고돈(010005)   CNT_6: 비생산돈(010006)
-- ============================================================
...
BEGIN
    ...
    WHERE MASTER_SEQ = P_MASTER_SEQ
      ...
      AND (GUBUN IN ('DOPE', 'DOPE_CHART') OR GUBUN LIKE 'DOPE_R_%');
    ...
    -- ================================================
    -- 4. 원인별 테이블 INSERT (GUBUN='DOPE_R_1', 'DOPE_R_2', ...)
    ...
    SELECT
        ...
        'DOPE_R_' || GRP_NO,  -- DOPE_R_1, DOPE_R_2, ...
        1,
        -- STR_1~STR_15: 원인코드
    ...
    FROM (
        ...
        FROM (
            SELECT
                REASON_CD, LAST_WEEK, LAST_MONTH,
                ROW_NUMBER() OVER(ORDER BY LAST_MONTH DESC, LAST_WEEK DESC, REASON_CD) AS RN
            FROM (
                -- 지난주+최근1개월 원인코드 병합 (FULL OUTER JOIN 효과)
                SELECT
                    MD.OUT_REASON_CD AS REASON_CD,
