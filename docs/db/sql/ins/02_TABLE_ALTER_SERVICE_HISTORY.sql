...
WHERE S.USE_YN = 'Y'
  AND S.INSPIG_YN = 'Y'
  AND S.INSPIG_FROM_DT <= TO_CHAR(SYSDATE + 9/24, 'YYYYMMDD')
  AND LEAST(S.INSPIG_TO_DT, NVL(S.INSPIG_STOP_DT, '99991231')) >= TO_CHAR(SYSDATE + 9/24, 'YYYYMMDD')
  ...
  AND S.INSPIG_REG_DT = (
      ...
      WHERE S2.FARM_NO = S.FARM_NO
        AND S2.USE_YN = 'Y'
        AND S2.INSPIG_YN = 'Y'
        AND S2.INSPIG_FROM_DT <= TO_CHAR(SYSDATE + 9/24, 'YYYYMMDD')
        AND LEAST(S2.INSPIG_TO_DT, NVL(S2.INSPIG_STOP_DT, '99991231')) >= TO_CHAR(SYSDATE + 9/24, 'YYYYMMDD')
  );
