...
| MAP_X | VARCHAR2(20) | | 맵정보X좌표 |
| MAP_Y | VARCHAR2(20) | | 맵정보Y좌표 |
...
PK: SEQ
인덱스:
  - PK_TS_INS_MASTER (SEQ) - UNIQUE (PK)
  - IDX_TS_INS_MASTER_01 (DAY_GB, INS_DT)
  - IDX_TS_INS_MASTER_02 (STATUS_CD)
  - UK_TS_INS_MASTER_01 (DAY_GB, REPORT_YEAR, REPORT_WEEK_NO) - UNIQUE
```

| 컬럼명 | 타입 | NULL | 설명 |
|--------|------|------|------|
| SEQ | NUMBER | NOT NULL | 일련번호 (PK) |
| DAY_GB | VARCHAR2(10) | | 기간구분 (WEEK:주간, MON:월간, QT:분기) |
| REPORT_YEAR | NUMBER | | 년도 |
| REPORT_WEEK_NO | NUMBER | | 주차 (1~53) 또는 월 (1~12) |
| INS_DT | VARCHAR2(8) | | 생성기준일 (YYYYMMDD) |
| DT_FROM | VARCHAR2(8) | | 리포트 시작일 (YYYYMMDD) |
| DT_TO | VARCHAR2(8) | | 리포트 종료일 (YYYYMMDD) |
| STATUS_CD | VARCHAR2(20) | | 상태 (READY:대기, RUNNING:실행중, COMPLETE:완료, ERROR:오류) |
| TARGET_CNT | NUMBER | | 대상 농장수 |
| COMPLETE_CNT | NUMBER | | 실행완료 농장수 |
| ERROR_CNT | NUMBER | | 오류 농장수 |
| START_DT | DATE | | 실행 시작일시 |
| END_DT | DATE | | 실행 종료일시 |
| ELAPSED_SEC | NUMBER | | 실행 소요시간(초) |
...
```
PK: MASTER_SEQ, FARM_NO
FK: MASTER_SEQ → TS_INS_MASTER.SEQ
인덱스:
  - PK_TS_INS_WEEK (MASTER_SEQ, FARM_NO) - UNIQUE (PK)
  - IDX_TS_INS_WEEK_01 (FARM_NO, MASTER_SEQ)
  - IDX_TS_INS_WEEK_02 (FARM_NO, REPORT_YEAR, REPORT_WEEK_NO)
  - UK_TS_INS_WEEK_TOKEN (SHARE_TOKEN) - UNIQUE
```

| 컬럼명 | 타입 | NULL | 설명 |
|--------|------|------|------|
| MASTER_SEQ | NUMBER | NOT NULL | 마스터 일련번호 (PK, FK) |
| FARM_NO | NUMBER | NOT NULL | 농장번호 (PK) |
| STATUS_CD | VARCHAR2(20) | | 상태 (READY:대기, RUNNING:실행중, COMPLETE:완료, ERROR:오류) |
| MODON_REG_CNT | NUMBER | | 현재모돈(등록모돈수) |
| MODON_REG_CHG | NUMBER | | 현재모돈 증감 (전주 대비) |
| MODON_SANGSI_CNT | NUMBER | | 상시모돈수 |
| MODON_SANGSI_CHG | NUMBER | | 상시모돈 증감 (전주 대비) |
| SHARE_TOKEN | VARCHAR2(64) | | 공유용 토큰 (SHA256 해시, 64자) |
| TOKEN_EXPIRE_DT | VARCHAR2(8) | | 토큰 만료일 (YYYYMMDD, 생성일 + 7일) |

> **참고**: TS_INS_WEEK 테이블에는 위 컬럼 외에도 각 프로세서에서 업데이트하는 LAST_* 컬럼들이 다수 존재합니다.
...
FK: MASTER_SEQ, FARM_NO → TS_INS_WEEK
인덱스:
  - PK_TS_INS_WEEK_SUB (MASTER_SEQ, FARM_NO, GUBUN, SUB_GUBUN, SORT_NO) - UNIQUE (PK)
  - IDX_TS_INS_WEEK_SUB_01 (MASTER_SEQ, FARM_NO, GUBUN)
  - IDX_TS_INS_WEEK_SUB_02 (MASTER_SEQ, FARM_NO, GUBUN, SUB_GUBUN)
```
...
| MASTER_SEQ | NUMBER | NOT NULL | 마스터 일련번호 (PK, FK) |
| FARM_NO | NUMBER | NOT NULL | 농장번호 (PK) |
| GUBUN | VARCHAR2(20) | NOT NULL | 데이터 구분코드 (PK) |
| SUB_GUBUN | VARCHAR2(20) | NOT NULL | 세부 구분코드 (PK) |
| SORT_NO | NUMBER | NOT NULL | 정렬순서 (PK) |
| CODE_1 | VARCHAR2(20) | | 1차 구분코드 |
| CODE_2 | VARCHAR2(20) | | 2차 구분코드 |
...
### TS_INS_JOB_LOG (스케줄러 실행 로그 테이블)

```
PK: SEQ
인덱스:
  - PK_TS_INS_JOB_LOG (SEQ) - UNIQUE (PK)
  - IDX_TS_INS_JOB_LOG_01 (MASTER_SEQ)
  - IDX_TS_INS_JOB_LOG_02 (JOB_NM, START_DT)
  - IDX_TS_INS_JOB_LOG_03 (STATUS_CD, START_DT)
  - IDX_TS_INS_JOB_LOG_04 (MASTER_SEQ, FARM_NO, STATUS_CD)
  - IDX_TS_INS_JOB_LOG_05 (DAY_GB, REPORT_YEAR, REPORT_WEEK_NO)
```
...
| FARM_NO | NUMBER | | 농장번호 (NULL=전체) |
| DAY_GB | VARCHAR2(10) | | 기간구분 (WEEK:주간, MON:월간, QT:분기) |
| REPORT_YEAR | NUMBER | | 리포트 년도 |
| REPORT_WEEK_NO | NUMBER | | 리포트 주차/월/분기 |
| JOB_NM | VARCHAR2(100) | | JOB 이름 |
| PROC_NM | VARCHAR2(100) | | 프로시저명 |
| STATUS_CD | VARCHAR2(20) | | 상태 (RUNNING:실행중, SUCCESS:성공, ERROR:오류) |
| PROC_CNT | NUMBER | | 처리 건수 |
| ERROR_CD | VARCHAR2(20) | | 오류 코드 |
| ERROR_MSG | VARCHAR2(4000) | | 오류 메시지 |
| START_DT | DATE | | 시작일시 |
| END_DT | DATE | | 종료일시 |
| ELAPSED_MS | NUMBER | | 소요시간(ms) |
