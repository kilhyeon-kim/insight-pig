...
│  - FK: FARM_NO → TS_INS_SERVICE.FARM_NO                      │
└─────────────────────────┬───────────────────────────────────┘
                          │ 1:N
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
| 테이블명 | 설명 | 용도 |
|----------|------|------|
| `TS_INS_FARM` | 농장별 리포트 테이블 | 주간/월간/분기 보고서 요약 데이터 |
| `TS_INS_FARM_SUB` | 리포트 상세 테이블 | 각 팝업별 리스트 데이터 |
...
```sql
-- 스케줄 실행여부 확인
SELECT INS_SCHEDULE_YN FROM TA_SYS_CONFIG WHERE SEQ = 1;

-- 스케줄 실행 중단
UPDATE TA_SYS_CONFIG SET INS_SCHEDULE_YN = 'N', LOG_UPT_DT = SYSDATE WHERE SEQ = 1;
```
...
| `INSPIG_TO_DT` | DATE | 서비스 종료일 | NULL이면 무기한 |
| `INSPIG_STOP_DT` | DATE | 서비스 중단일 | NULL이면 정상 |
| `WEB_PAY_YN` | VARCHAR2(1) | 웹결재 여부 | Y/N |
| `USE_YN` | VARCHAR2(1) | 사용여부 | 기본값 'Y' |

**스케줄 대상 농장 조건:**
```sql
-- 아래 조건을 모두 만족하는 농장만 리포트 생성
WHERE INSPIG_YN = 'Y'                                    -- 서비스 신청됨
  AND USE_YN = 'Y'                                       -- 사용중
  AND (INSPIG_TO_DT IS NULL OR INSPIG_TO_DT >= SYSDATE)  -- 종료일 미경과
  AND INSPIG_STOP_DT IS NULL                             -- 중단되지 않음
```
...
```sql
-- 특정 리포트의 오류 농장 조회
SELECT F.FARM_NO, F.FARM_NM, F.STATUS_CD,
       L.PROC_NM, L.ERROR_CD, L.ERROR_MSG, L.START_DT
FROM TS_INS_FARM F
LEFT JOIN TS_INS_JOB_LOG L
  ON F.MASTER_SEQ = L.MASTER_SEQ
  AND F.FARM_NO = L.FARM_NO
  AND L.STATUS_CD = 'ERROR'
WHERE F.MASTER_SEQ = :master_seq
  AND F.STATUS_CD = 'ERROR';
```

---

### 2.5 TS_INS_FARM (농장별 리포트)

농장별 주간/월간/분기 보고서의 요약 데이터를 저장합니다.
...
| `SEQ` | 일련번호 | PK |
| `WK_DATE` | 날짜 | |
| `SIGUNGU_CD` | 시군구코드 | 행정표준코드, UK |
| `NX`, `NY` | 기상청 격자좌표 | |
| `WEATHER_CD` | 날씨코드 | sunny, cloudy, rainy, snow |
| `TEMP_HIGH` | 최고기온(℃) | |
| `TEMP_LOW` | 최저기온(℃) | |
| `RAIN_PROB` | 강수확률(%) | |
...
**TS_INS_FARM과 조인:**
```sql
SELECT F.FARM_NM, W.WEATHER_CD, W.TEMP_HIGH, W.TEMP_LOW
FROM TS_INS_FARM F
JOIN TM_WEATHER W ON F.SIGUNGU_CD = W.SIGUNGU_CD AND W.WK_DATE = :target_date
WHERE F.MASTER_SEQ = :master_seq;
```
...
| TS_INS_FARM | 68,000건 | 마스터 1건 × 농장 1,000개 |
| TS_INS_FARM_SUB | 7,000,000건 | FARM 1건 × 약 100~150건 |
...
| TS_INS_FARM | IDX(FARM_NO, MASTER_SEQ) | 농장별 조회 |
| TS_INS_FARM_SUB | IDX(MASTER_SEQ, FARM_NO, GUBUN) | 팝업 데이터 조회 |
...
2. **오류 로깅 통합**: `TS_INS_FARM.ERROR_MSG` 제거, `TS_INS_JOB_LOG`에서 통합 관리
